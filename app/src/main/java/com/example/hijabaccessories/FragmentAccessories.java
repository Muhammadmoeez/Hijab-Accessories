...
public class FragmentAccessories extends Fragment {
    ...
    View viewAccessories;
    RecyclerView.LayoutManager layoutManager;
    FirebaseDatabase firebaseDatabase;
    Query databaseReferenceAccessories;

    private RecyclerView myRecyclerViewAccessories;
    FirebaseRecyclerAdapter<ProductModel, FragmentAccessories.MyAllPublicViewHolder> adapter;

    public FragmentAccessories() {

        firebaseDatabase = FirebaseDatabase.getInstance();
        databaseReferenceAccessories = firebaseDatabase.getReference("AllProduct")
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        viewAccessories = inflater.inflate(R.layout.fragment_accessories, container,false);
        myRecyclerViewAccessories = (RecyclerView) viewAccessories.findViewById(R.id.myAllAccessoriesProductRecyclerView);
        layoutManager = new GridLayoutManager(getActivity(),2);
        myRecyclerViewAccessories.setLayoutManager(layoutManager);
    ...
    private void showList() {
            ...
            FirebaseRecyclerOptions options = new FirebaseRecyclerOptions.Builder<ProductModel>()
                    .setQuery(databaseReferenceAccessories, ProductModel.class).build();

            adapter = new FirebaseRecyclerAdapter<ProductModel, FragmentAccessories.MyAllPublicViewHolder>(options) {
                @Override
                protected void onBindViewHolder(@NonNull FragmentAccessories.MyAllPublicViewHolder myAllPublicViewHolder, int i, @NonNull ProductModel productModel) {
                ...
                @Override
                public FragmentAccessories.MyAllPublicViewHolder onCreateViewHolder(@NonNull  ViewGroup viewGroup, int viewType) {

                    View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.public_post, viewGroup, false);
                    return new FragmentAccessories.MyAllPublicViewHolder(view);
                }
            };
            ...
            myRecyclerViewAccessories.setAdapter(adapter);
